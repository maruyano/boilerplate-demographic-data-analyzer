...
def calculate_demographic_data(print_data=True):
    ...
    higher_education = len(df.loc[((df['education'] == 'Masters') | (df['education'] == 'Doctorate') | (df['education'] == 'Bachelors') )])
    lower_education = len(df.loc[((df['education'] != 'Masters') & (df['education'] != 'Doctorate') & (df['education'] != 'Bachelors') )])

    # percentage with salary >50K
    higher_education_rich = round(len(df.loc[(((df['education'] == 'Masters') 
                                         | (df['education'] == 'Doctorate') 
                                         | (df['education'] == 'Bachelors'))
                                         & (df['salary'] == '>50K') )]) \
                                            / higher_education * 100, 1)
    lower_education_rich = round(len(df.loc[( (df['education'] != 'Masters') 
                                         & (df['education'] != 'Doctorate') 
                                         & (df['education'] != 'Bachelors')
                                         & (df['salary'] == '>50K'))
    ...
    rich_percentage = ( (df['salary'] == '>50K')
                       & (df['hours-per-week'] == df['hours-per-week']\
                          .min()) ).sum() / num_min_workers * 100
